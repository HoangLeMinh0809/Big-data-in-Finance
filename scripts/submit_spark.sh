...
case "$JOB_TYPE" in
  ...
  reconcile-serving)
    ...
    ;;
  *)
    echo "Usage: $0 [weather|openaq|sentinel5p|maiac|weather-ingest|openaq-ingest|sentinel5p-ingest|maiac-ingest|cassandra-weather|cassandra-openaq|ensure-iceberg|maintenance-iceberg|reconcile-serving]"
    exit 1
    ;;
esac
